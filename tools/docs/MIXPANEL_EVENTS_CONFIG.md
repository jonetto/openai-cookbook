...
These properties were previously only available via the Engage API (`--enrich`). Now stamped directly on events:
...
| `Calcula Perc AGIP` / `Calcula Perc ARBA` | Tax perception flags | |

### Old vs New Event Property Count

| Format | Properties per Login | `product_id` present | `CUIT` present |
| --- | --- | --- | --- |
| Old (pre-KAN-12024) | ~35 | No | No |
| New (post-KAN-12024) | ~98 | Yes (array) | Yes |

### Key Relationships

- `product_id` = `empresa.IdEmpresa` (product subscription) — validated 99.9%
- `CUIT` = `facturacion.CUIT` (billing entity) — validated 95.3%
- `company_id` (event property) = still `idEmpresa` (unchanged)
- `company` group assignment = `CUITFacturacion` (changed at `set_group()` level, not event property)

### Handling Mixed Data During Rollout

```python
# Detect new vs old event format
if event["properties"].get("product_id"):
    ...
    cuit = event["properties"].get("CUIT")
else:
    # Old format — need --enrich or Engage API for company properties
